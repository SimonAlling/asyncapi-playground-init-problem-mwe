...
export function debounce<_>(
  ...
  return (...args: any[]) => {
    ...
    onStart();
    const setTimeout = window.setTimeout // Hotfix for `TS2322: Type 'Timeout' is not assignable to type 'number'`; without this, `setTimeout` is assumed to be the Node one, which doesn't return `number`.
    timeout = setTimeout(() => {
      timeout = undefined;
      func(...args);
      onCancel();
